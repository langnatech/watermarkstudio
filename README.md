...
| 16 KB 页面 | NDK r28 + OpenCV **4.12+** | Play 预检 `libopencv_java4.so`；自研 `removal_native` 见 `CMakeLists.txt` `ANDROID_SUPPORT_FLEXIBLE_PAGE_SIZES` |
...
1. 首页 → **消除水印** → **进入去水印工作台**
2. **视频预览**：首帧 + [`RemovalPreviewHelper.renderVideoPreview`](app/src/main/java/com/watermarkstudio/removal/preview/RemovalPreviewHelper.kt) 显示修复效果；用笔刷直接涂抹水印笔画，滑块控制 `brushRadiusPct`（媒体宽度的 0.5–8%，默认 2.5%）；笔画变化后约 400ms debounce 再跑 PatchMatch 预览
...
| **视频** | **源帧率**（≤60fps，15s 内 900 帧预算）→ 缓存 mask + 轻量 PatchMatch 逐帧 → H.264 + 原声 | **源帧率不降帧**（≤60fps，≤5min）→ 缓存 mask + PatchMatch（更多迭代） |
| **预览** | 图片/视频首帧同一笔刷 mask + PatchMatch 预览（[`RemovalPreviewHelper`](app/src/main/java/com/watermarkstudio/removal/preview/RemovalPreviewHelper.kt)） | 同左 |
...
| 流式视频修复 | 跳过光流/中值，[`FrameInpaintBlender`](app/src/main/java/com/watermarkstudio/removal/video/FrameInpaintBlender.kt) 缓存 mask + `InpaintTarget.VIDEO` 轻量 PatchMatch |
...
| 轻微平移背景视频 | 逐帧 PatchMatch | 逐帧 PatchMatch + 原声 |
| 运动剧烈视频 | 可能残留 | 逐帧 PatchMatch（无跨帧融合） |
| 无音轨视频 | 仅视频轨 | 仅视频轨 |
| 模拟器 | 视频禁用提示 | 同左 |
