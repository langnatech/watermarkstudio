...
1. 首页 → **消除水印** → **进入去水印工作台**
2. **视频预览**：首帧 + [`RemovalPreviewHelper.renderVideoPreview`](app/src/main/java/com/watermarkstudio/removal/preview/RemovalPreviewHelper.kt) 显示修复效果；**消除区域大小**滑块（0.3x–3x）+ 拖动绿色框定位
3. 支持**图片与视频**（模拟器/低内存设备视频会提示 `error_remove_video_not_supported`）
4. 绿色预览框与处理区域使用同一 `RemovalRegion` 比例
...
| **视频** | **源帧率**（≤60fps，15s 内 900 帧预算）→ 光流 + TELEA → H.264 + 原声 | **源帧率不降帧**（≤60fps，≤5min，仅加长处理时间）→ Pro NS/seamlessClone |
| **预览** | 图片/视频首帧 inpaint 预览（[`RemovalPreviewHelper`](app/src/main/java/com/watermarkstudio/removal/preview/RemovalPreviewHelper.kt)） | 同左 |
